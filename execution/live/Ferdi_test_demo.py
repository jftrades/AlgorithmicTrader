...
# Get and verify API keys (DEMO mode)
api_key = os.getenv("BYBIT_DEMO_API_KEY")
api_secret = os.getenv("BYBIT_DEMO_API_SECRET")

print("🔑 Checking API Keys (DEMO mode)...")
print(f"API Key found: {api_key is not None}")
print(f"API Secret found: {api_secret is not None}")

if not api_key or not api_secret:
    print("❌ ERROR: BYBIT_DEMO_API_KEY or BYBIT_DEMO_API_SECRET not found in .env file")
    print("📝 Make sure your .env file is in the project root and contains:")
    print("   BYBIT_DEMO_API_KEY=your_demo_key_here")
    print("   BYBIT_DEMO_API_SECRET=your_demo_secret_here")
    exit(1)

print("✅ API Keys loaded successfully (DEMO)")
...
config_node = TradingNodeConfig(
    ...
    data_clients={
        BYBIT: BybitDataClientConfig(
            api_key=None,  # 'BYBIT_DEMO_API_KEY' env var
            api_secret=None,  # 'BYBIT_DEMO_API_SECRET' env var
            base_url_http=None,  # Override with custom endpoint
            instrument_provider=InstrumentProviderConfig(load_all=True),
            product_types=product_types,
            demo=True,  # Using DEMO API
            testnet=False,  # Not using testnet
