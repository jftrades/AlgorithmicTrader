...
# Get and verify API keys (TESTNET mode)
api_key = os.getenv("BYBIT_TESTNET_API_KEY")
api_secret = os.getenv("BYBIT_TESTNET_API_SECRET")

print("Checking API Keys (TESTNET mode)...")
print(f"API Key found: {api_key is not None}")
print(f"API Secret found: {api_secret is not None}")

if not api_key or not api_secret:
    print("ERROR: BYBIT_TESTNET_API_KEY or BYBIT_TESTNET_API_SECRET not found in .env file")
    print("Make sure your .env file is in the project root and contains:")
    print("   BYBIT_TESTNET_API_KEY=your_testnet_key_here")
    print("   BYBIT_TESTNET_API_SECRET=your_testnet_secret_here")
    exit(1)

print("API Keys loaded successfully (TESTNET)")
...
config_node = TradingNodeConfig(
    ...
    data_clients={
        BYBIT: BybitDataClientConfig(
            api_key=None,  # 'BYBIT_TESTNET_API_KEY' env var
            api_secret=None,  # 'BYBIT_TESTNET_API_SECRET' env var
            base_url_http=None,  # Override with custom endpoint
            instrument_provider=InstrumentProviderConfig(load_all=True),
            product_types=product_types,
            demo=False,  # Not using DEMO API
