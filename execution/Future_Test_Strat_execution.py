...
symbol = Symbol("AINUSDT")
venue = Venue("BINANCE")
instrument_id = InstrumentId(symbol, venue)
instrument_id_str = "AINUSDT-PERP.BINANCE"
bar_type_str_for_configs = "AINUSDT-PERP.BINANCE-5-MINUTE-LAST-EXTERNAL"
...
# Lade alle Instrumente aus der CSV
def load_all_instruments_from_csv():
    instruments = []
    csv_path = Path(__file__).resolve().parents[1] / "data" / "download" / "new_binance_perpetual_futures.csv"
    with open(csv_path, "r") as f:
        ...
        for row in reader:
            ...
            instruments.append({
                ...
                "bar_type_str": f"{symbol}-PERP.BINANCE-5-MINUTE-LAST-EXTERNAL"
            })
    return instruments

# Lade alle Instrumente
all_instruments = load_all_instruments_from_csv()
print(f"[INFO] {len(all_instruments)} Instrumente geladen")

# Teste erstmal nur mit einem Instrument
first_instrument = all_instruments[0]  # Nimm das erste Instrument zum Testen

# Erstelle bar_types Liste für alle Instrumente
all_bar_types = [inst["bar_type_str"] for inst in all_instruments]

# DataConfig - ALLE Instrumente
data_config = BacktestDataConfig(
    data_cls="nautilus_trader.model.data:Bar",
    catalog_path=catalogPath,
    bar_types=all_bar_types  # Alle Bar-Type-Strings
)
...
# StrategyConfig - zurück zum Original-Format
strategy_config = ImportableStrategyConfig(
    strategy_path="strategies.Future_Test_Strat:RSISimpleStrategy",
    config_path="strategies.Future_Test_Strat:RSISimpleStrategyConfig",
    config={
        "instrument_id": first_instrument["instrument_id_str"],  # Einzelnes Instrument
        "bar_type": first_instrument["bar_type_str"],  # Einzelner Bar-Type
        "trade_size": "10",
        "rsi_period": 14,
        "rsi_overbought": 0.8,
        "rsi_oversold": 0.2,
