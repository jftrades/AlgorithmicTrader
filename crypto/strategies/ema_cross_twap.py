...
class EMACrossTWAP(Strategy):
    ...
    def on_bar(self, bar: Bar) -> None:
        ...
        self._plot_log.append({
            "timestamp": unix_nanos_to_dt(bar.ts_event),
            "open": bar.open,
            "high": bar.high,
            "low": bar.low,
            "close": bar.close,
    ...
    def on_stop(self) -> None:
        ...
        df = pd.DataFrame(self._plot_log)
        df.to_csv("backtest_plot_data.csv", index=False)
        self.log.info("Plot-Log erfolgreich gespeichert: backtest_plot_data.csv", LogColor.GREEN)
