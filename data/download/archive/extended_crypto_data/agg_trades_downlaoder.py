...
class AggTradesDownloader:
    def __init__(self, symbol, start_date, end_date, base_data_dir):
        ...
        self.temp_raw_download_dir = Path(base_data_dir) / "temp_aggtrades_downloads"
        self.processed_dir = Path(base_data_dir) / f"processed_aggtrades_data_{self.start_date_str}_to_{self.end_date_str}" / "csv"
    ...
    def run(self):
        ...
        dumper = BinanceDataDumper(
            ...
            data_type="aggTrades",     # AggTrades-Daten
        )
        ...
        if not all_csvs:
            print("❌ Keine AggTrades-CSVs gefunden.")
        ...
        combined_path = self.processed_dir / f"{self.symbol}_AGGTRADES_{self.start_date_str}_to_{self.end_date_str}.csv"
        ...
        print(f"✅ AggTrades CSV gespeichert: {combined_path}")
